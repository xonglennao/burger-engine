...
public:
  ...
  void        check_in (const vec3* p, unsigned int iCount);
...
//added by Baptiste Malaga
template <int d>
void Miniball<d>::check_in (const vec3* p, unsigned int iCount)
{
  for(unsigned int i = 0; i < iCount; ++i)
  {
	L.push_back(Point<3>((float*)p[i]));
  }
}
